...
-normalise by population /100 000 or /10 000 inhabitants

-Katja will send new numbers which take into account the prescreening policy

-Johanna will get current Finnish data
...
if (!file.exists(file)) {
  ...
  per_donor <- donation %>% filter(Year == "2018") %>%
    ...
    summarise(
      ...
      #MeanHb = mean(Hb,na.rm = TRUE), #these need to count 
...
p3 <- p3 + geom_point(position=position_dodge(width = dodge_width),shape="diamond",size=3)
